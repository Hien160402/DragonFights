...
public class Vegito : CharacterController
{
    public bool isSuperSaiyan = false;
    public GameObject vegitoSuperSaiyanPrefab;
    public void Start()
    {
    ...
    private void TransformToSuperSaiyan()
    {
        ...
        GameObject vegitoSuperSaiyan = Instantiate(vegitoSuperSaiyanPrefab, transform.position, Quaternion.identity);
        Vegito vegitoSuperSaiyanScript = vegitoSuperSaiyan.GetComponent<Vegito>();
        vegitoSuperSaiyanScript.health = health;
        vegitoSuperSaiyanScript.damage = damage;
        vegitoSuperSaiyanScript.rage = rage;
        vegitoSuperSaiyan.layer = gameObject.layer;
        if (isLayer == isEnemy)
        {
            vegitoSuperSaiyan.name = "Enemy";
        }
        else
        {
            vegitoSuperSaiyan.name = "Player";
        }
